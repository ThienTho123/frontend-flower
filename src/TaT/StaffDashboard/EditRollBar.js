...
const EditRollBar = () => {
  ...
  const fetchRollBarDetails = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/staffrollbar/${id}`,
        {
          headers: {
  ...
  const confirmSubmit = async () => {
    try {
      ...
      await axios.put(`http://localhost:8080/staffrollbar/${id}`, rollBarData, {
      ...
      setIsModalOpen(false);
      setErrorMessage(error.response?.data || "Có lỗi xảy ra khi cập nhật Roll Bar!");
      setIsErrorModalOpen(true);
    }
  };
