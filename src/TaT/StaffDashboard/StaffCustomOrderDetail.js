...
const StaffCustomOrderDetail = () => {
  ...
  const fetchOrderDetail = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:8080/api/v1/staff/custom/${id}`, {
  ...
  const handleConfirmAction = async () => {
    try {
      if (modalMessage.includes("CHẤP NHẬN")) {
        ...
        await axios.put(
          `http://localhost:8080/api/v1/staff/custom/${id}/accept`,
          acceptDTO,
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
        ...
        // Xử lý success
        await axios.put(
          `http://localhost:8080/api/v1/staff/custom/${id}/success`,
          {},
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
        setIsModalOpen(false);
        ...
        // Xử lý decline
        await axios.put(
          `http://localhost:8080/api/v1/staff/custom/${id}/decline`,
          {},
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
        setIsModalOpen(false);
        ...
        // Xử lý cancel
        await axios.put(
          `http://localhost:8080/api/v1/staff/custom/${id}/cancel`,
          {},
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
        setIsModalOpen(false);
    ...
    <div className="Staff-custom-order-detail-container">
      <div className="Staff-custom-order-detail-header">
        <img
          ...
          onClick={() => navigate("/StaffCustomOrders")}
        />
        <h2 className="Staff-custom-order-detail-title">Chi Tiết Đơn Hàng Custom #{orderDetail.customID}</h2>
      </div>

      {/* Thông tin đơn hàng */}
      <div className="Staff-custom-order-detail-card">
        <h3 className="Staff-custom-order-detail-card-title">Thông Tin Đơn Hàng</h3>
        <div className="Staff-custom-order-detail-info-grid">
          <div className="Staff-custom-order-detail-info-column">
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Khách hàng:</strong>
              <span className="Staff-custom-order-detail-info-value">{orderDetail.name || "N/A"}</span>
            </p>
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Số điện thoại:</strong>
              <span className="Staff-custom-order-detail-info-value">{orderDetail.phoneNumber || "N/A"}</span>
            </p>
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Địa chỉ giao hàng:</strong>
              <span className="Staff-custom-order-detail-info-value--secondary">{orderDetail.deliveryAddress || "N/A"}</span>
            </p>
          </div>
          <div className="Staff-custom-order-detail-info-column">
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Ngày đặt:</strong>
              <span className="Staff-custom-order-detail-info-value--secondary">{formatDateTime(orderDetail.date)}</span>
            </p>
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Trạng thái:</strong>
              {renderStatus(orderDetail.condition)}
            </p>
            <p className="Staff-custom-order-detail-info-item">
              <strong className="Staff-custom-order-detail-info-label">Tổng tiền hiện tại:</strong>
        ...
        {orderDetail.note && (
          <div className="Staff-custom-order-detail-note">
            <p>
              <strong className="Staff-custom-order-detail-note-label">Ghi chú:</strong>
              <span className="Staff-custom-order-detail-note-text">{orderDetail.note}</span>
            </p>
          </div>
        )}
      </div>
      ...
      {orderDetail.image && (
        <div className="Staff-custom-order-detail-card">
          <h3 className="Staff-custom-order-detail-card-title">Hình Ảnh Đơn Hàng</h3>
          <img
      ...
      {existingDetails.length > 0 && (
        <div className="Staff-custom-order-detail-card">
          <h3 className="Staff-custom-order-detail-card-title">Chi Tiết Đã Được Chấp Nhận</h3>
          <div className="Staff-custom-order-detail-existing-details">
            {existingDetails.map((detail, index) => (
              <div key={index} className="Staff-custom-order-detail-existing-item">
                <div className="Staff-custom-order-detail-existing-dot"></div>
                <span className="Staff-custom-order-detail-existing-text">{renderDetailInfo(detail)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {orderDetail.condition === "PROCESSING" && (
        <div className="Staff-custom-order-detail-card">
          <h3 className="Staff-custom-order-detail-card-title">Chấp Nhận Đơn Hàng</h3>
          
          {/* Upload ảnh */}
          <div className="Staff-custom-order-detail-form-section">
            <label className="Staff-custom-order-detail-form-label">
              Tải ảnh sản phẩm lên: <span className="Staff-custom-order-detail-form-required">*</span>
            </label>
            ...
            {imageUrl && (
              <div className="Staff-custom-order-detail-image-preview">
                <p className="Staff-custom-order-detail-preview-label">Ảnh đã tải:</p>
                <img
                  src={imageUrl}
                  alt="Preview"
  ...
  <div className="Staff-custom-order-detail-form-section">
    <div className="Staff-custom-order-detail-products-header">
      <h4 className="Staff-custom-order-detail-products-title">
        Chi Tiết Sản Phẩm: <span className="Staff-custom-order-detail-form-required">*</span>
      </h4>
      <button
    ...
    {newDetails.length === 0 ? (
      <div className="Staff-custom-order-detail-empty-state">
        <p className="Staff-custom-order-detail-empty-text">Chưa có detail nào. Nhấn "Thêm Detail" để bắt đầu.</p>
      </div>
    ) : (
      <div className="Staff-custom-order-detail-products-list">
        {newDetails.map((detail, index) => (
          <div key={index} className="Staff-custom-order-detail-product-item">
            {/* Loại sản phẩm */}
          ...
          <div className="Staff-custom-order-detail-total">
            <h4 className="Staff-custom-order-detail-total-text">
              💰 Tổng tiền: <span className="Staff-custom-order-detail-total-amount">{totalPrice.toLocaleString()} VND</span>
            </h4>
          </div>
